...
$userMenu = array(
    ...
    array('label' => Yii::t('app', 'Delete Ads'), 'url' => array('/advertise/deleteAll'),'active' => ($module == 'admin') ? true : null, 'visible' => $isAdmin),
...
<?php //echo $backgroundImg; ?>
    ...
    <?php
        $ads = Yii::app()->db->createCommand('select * from x2_advertise order by budget desc')->queryAll();
        if ($l = count($ads)) {
            $choose = rand(0, $l > 3? 3 : $l-1);
            echo "<div id='advertisementTopBanner-".$ads[$choose]['id']."' style='padding:5px;width:99%;height:150px;overflow:hidden;'>";
            echo "<iframe src='".$ads[$choose]['url']."' width='100%'></iframe></div>";
        }
    echo $content; ?>
	<!--
